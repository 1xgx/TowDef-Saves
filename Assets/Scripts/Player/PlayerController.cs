...
public class PlayerController : MonoBehaviour
{
        ...
        if (Input.touchCount > 0)
        {
            ...
            if (touch.phase == UnityEngine.TouchPhase.Began)
            {
                ...
                if (Physics.Raycast(ray, out hit) && hit.collider != null)
                {
                    ...
                    {
                        if (_selectedObject.GetComponent<VehicleCellMovement>())
                        {
                            GameObject newVehicle = _selectedObject;
                            HoldInputControler newHoldInput = gameObject.GetComponent<HoldInputControler>();
                            newHoldInput.setValue(newVehicle);
                            newVehicle.GetComponent<CellVision>();
                        }
    ...
    {
        CellVisability CellVisability = new CellVisability();// the next day;
        _selectedObject.GetComponent<CellVision>();
    ...
    public void ObjectSetPositionOnHexagon(string message)
    {
        ...
        if (isNotNull)
        {
            ...
            {
                if (_choosedObject.GetComponent<UnitCost>().cost <= Money)
                {
                    ...
                    Money -= newBuild.GetComponent<UnitCost>().cost;
                    UpdateMoney();
