...
public class GameManager : MonoBehaviour
{
    ...
    public Transform randomElectroStationForMissle() 
    {
        if (Towers.Count <= 0)
        {
            Debug.Log("I'm so SORRY u LOST, but U CAN TRY AGAIN ! ;) ");
            return null;
        }
    ...
    private IEnumerator Battle(float time)
    {
        ...
        time = 0;
        UIManager.Instance.UpdateBattleTimer(time);
        if (time == 0) StopWave();
        StopCoroutine(Battle(time));
    }
    ...
    private IEnumerator TransitionToDay()
    {
        ...
        while (time < 1f)
        {
            time += Time.deltaTime / _transitionDuration;
            _directionalLight.color = Color.Lerp(initialColor, _dayColor, time);
            _directionalLight.intensity = Mathf.Lerp(initialIntensity, 1f, time);
            UnityEngine.RenderSettings.skybox.Lerp(_skyBoxNight, _skyBoxDay, time);
